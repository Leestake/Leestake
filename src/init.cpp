...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("Leestake version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  Leestaked [options]                     " + "\n" +
                  "  Leestaked [options] <command> [params]  " + _("Send command to -server or Leestaked") + "\n" +
                  "  Leestaked [options] help                " + _("List commands") + "\n" +
                  "  Leestaked [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "Leestake:") && !boost::algorithm::istarts_with(argv[i], "LEESoin:"))
                fCommandLine = true;
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: Leestake.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: Leestaked.pid)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    //
    // LEESoin: -mintxfee and -minrelaytxfee options of bitcoin disabled
    ...
    if (mapArgs.count("-checkpointkey")) // LEESoin: checkpoint master priv key
    {
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Leestake is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("Leestake version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "Leestake server starting\n");
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of Leestake") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart Leestake to complete") << "\n";
