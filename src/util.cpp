...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Leestake
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Leestake
    // Mac: ~/Library/Application Support/Leestake
    // Unix: ~/.Leestake
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Leestake";
#else
    fs::path pathRet;
...
{
    namespace fs = boost::filesystem;
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LEESoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LEESoin
    // Mac: ~/Library/Application Support/LEESoin
    // Unix: ~/.LEESoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LEESoin";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Leestake.conf"));
    if (!pathConfigFile.is_complete()) pathConfigFile = GetDataDir(false) / pathConfigFile;

    // Load old config file if present
    if (mapArgs.count("-conf") == 0 && !boost::filesystem::exists(pathConfigFile)) {
        boost::filesystem::path pathOldConfigFile = GetDataDir(false) / "LEESoin.conf";
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Leestaked.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Leestake will not work properly.");
...
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments)
{
    ...
    ss << "/";
    ss << "Leestake:" << FormatVersion(Leestake_VERSION);
    ss << "(" << CLIENT_BUILD << ")/";
    return ss.str();
}
